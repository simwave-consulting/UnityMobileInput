...
namespace Mopsicus.Plugins
{
    ...
    public class MobileInputField : MobileInputReceiver
    {
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogError(string.Format("No found InputField for {0} MobileInput", name));
        ...
        private IEnumerator InitializeOnNextFrame()
        {
            ...
            CreateNativeEdit();
            SetTextNative(_inputObjectText.text);
...
#endif
            if (_isMobileInputCreated && InputField != null)
            {
#if !UNITY_EDITOR
                int touchCount = Input.touchCount;
                if (touchCount > 0)
                {
                    Rect inputRect = _inputObjectText.rectTransform.rect;
        ...
        private void PrepareNativeEdit()
        {
            ...
            _config.CharacterLimit = InputField.characterLimit;
            Rect rect = GetScreenRectFromRectTransform(_inputObjectText.rectTransform);
            float ratio = rect.height / _inputObjectText.rectTransform.rect.height;
            _config.FontSize = (_inputObjectText.fontSize) * ratio;
            _config.TextColor = _inputObjectText.color;
            _config.Align = _inputObjectText.alignment.ToString();
            _config.ContentType = InputField.contentType.ToString();
            _config.BackgroundColor = InputField.colors.normalColor;
            _config.Multiline = (InputField.lineType == InputField.LineType.SingleLine) ? false : true;
            _config.KeyboardType = InputField.keyboardType.ToString();
            _config.InputType = InputField.inputType.ToString();
        }
        ...
        public void SetRectNative(RectTransform inputRect)
        {
            ...
            data["x"] = InvariantCultureString(rect.x / Screen.width);
            data["y"] = InvariantCultureString(rect.y / Screen.height);
            data["width"] = InvariantCultureString(Visible ? (rect.width / Screen.width) : 0);
            //data["width"] = InvariantCultureString(rect.width / Screen.width);
            data["height"] = InvariantCultureString(rect.height / Screen.height);
            Execute(data);
        }
        ...
        public void SetVisible(bool isVisible)
        {
            ...
            InputField.interactable = Visible;
            _inputObjectText.enabled = !Visible;
